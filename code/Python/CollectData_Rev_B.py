...
PRIVATE_POWER_SERVICE = 'ACD63933AE9C393DA313F75B3E5F9A8C'
...
def on_read_data(e):
		...
		if len(splitList) == 3:    
			...
			if splitList[1] == '0011':   # Check if it's the temperature
				...
				if len(temperature) > 2:
					foundTemp = 1
			if splitList[1] == '0019':   # Check if it's the TX Power
				txPower = splitList[2].strip('.')
				if len(txPower) > 1:
					foundPower = 1
	...
	if foundData:
		...
		if foundPower:
			readPower = int(txPower, 16)
			if readPower > 0:
				txPower = readPower
			else:
				txPower = 0;
			print "txPower = %d" % txPower

		dataFile = open(DataPath + BTA + '.txt', 'a') 
		print >> dataFile, "%f, %4d, %4d, %4d, %4d, %4d, %4d, %4d, %1.2f, %3.1f, %d" % (time.time(), sensor[0], sensor[1], sensor[2], sensor[3], sensor[4], sensor[5], sensor[6], Vbat, Temperature, txPower)
		dataFile.flush()
		dataFile.close()
